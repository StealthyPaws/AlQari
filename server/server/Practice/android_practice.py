...
DB_PATH = r"/content/drive/MyDrive/server/Practice/makharij_practice.db"
TMP_DIR = r"/content/drive/MyDrive/server/Practice/tmp_audio"
...
# ------------------------------
# Flask Endpoint (Base64 Input/Output)
# ------------------------------
ALLOWED_MIME_TO_EXT = {
    "audio/wav": ".wav",
    "audio/x-wav": ".wav",
...
def mime_to_ext(mime: str):
    return ALLOWED_MIME_TO_EXT.get(mime.lower(), ".wav")
...
def practice():
    try:
        ...
        if not data or "audio_b64" not in data or "gt" not in data:
            return jsonify({"error": "audio_b64 and gt are required"}), 400
